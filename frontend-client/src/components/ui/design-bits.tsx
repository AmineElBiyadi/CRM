...
type Stage = "froid" | "tiede" | "chaud" | "negociation" | "cloture" | "perdu";
...
export function SoftBadge({ children, tone = "neutral", className }: { children: ReactNode; tone?: "neutral" | "success" | "warn" | "danger" | "info"; className?: string }) {
  ...
  return <span className={cn("inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-medium", tones[tone], className)}>{children}</span>;
}
